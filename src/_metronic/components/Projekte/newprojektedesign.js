...
export default function Projekte() {
  ...
  const getStatusWiseData = async () => {
    try {
      const response = await ApiGet("vattenfall/getConformLead");
  ...
  const onDragEnd = async (result) => {
    ...
    if(source.droppableId?.startsWith("TERMINIERT") && destination.droppableId=="offen") return
                ...
                {(provided) => (
                  ...
                  >
                    ...
                    <div className="projeckt-ticket-main-body">
                      {getTicketsPerStatus(status.status)?.map((item, idx) => (
