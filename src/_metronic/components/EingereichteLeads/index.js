...
import AddMonestizel from "./leadsModel";
import { Modal } from "react-bootstrap";
import DatePicker from "react-datepicker";
import "react-datepicker/dist/react-datepicker.css";
import ConfirmationModal from "../Verfügbarkeit/Modal";
import WorkingDays from "../Verfügbarkeit/WorkingDays";
import {
...
export default function EingereichteLeads() {
  ...
  const handleDatePercentage = (datesData) => {
    ...
    const apiMonth =
      new Date([...new Set(datesData?.flat())][0]).getMonth() + 1;
    const allRanges = datesData
      .map((range) => generateDateRange(range[0], range[1]))
      .flat();
    if (!datesData?.length) {
      setCurrentDatePercentage(0);
    } else {
      setCurrentDatePercentage(
        calculateDatePercentage([...new Set(allRanges)])
      );
    }
  };
  useEffect(() => {
    handleGetDates();
  }, [dates.startDate, id, dates.endDate]);

  const dateIsAvailable = useCallback(
    (strDate) => {
      return isAvailable?.some(
  ...
  const handleGetDates = async () => {
    try {
      const response = await ApiGet(
        `dateSelect/getSelectDate?startDate=${dates?.startDate}&endDate=${
          dates?.endDate
        }${`&_id=${id || userInfo?._id}`}`
      );
      ...
      const groupedDateNotAvailable = groupDateRanges(
        dateNotAvailable?.date || []
      );
      const groupedAvailableTravel = groupDateRanges(
        isAvailableTravel?.date || []
      );
      ...
      const initialValue = [
        ...groupedAvailable,
        ...groupedAvailableTravel,
        ...groupedDateNotAvailable,
      ];
  ...
  const hanldeValidation = () => {
    ...
    let isValid = true;
    if (updatedLead?.trim() == "") {
      newerror["monestiziel"] = "Bitte geben Sie etwas ein";
      isValid = false;
    }
    setError(newerror);
    return isValid;
    ...
    try {
      if (id) {
        let resp = await ApiGet(
          `straper/getStraReportById?id=${id}${`&startDate=${
        ...
        let resp = await ApiGet(
          `straper/getStraReport?startDate=${
            firstDate.toISOString().split("T")[0]
          }${`&endDate=${endendDate.toISOString().split("T")[0]}`}`
        );
        setLeadData(resp?.data?.payload);
  ...
  const graphData = async () => {
    try {
      if (id) {
        let resp = await ApiGet(`straper/getStrapRepoMonById?id=${id}`);
        setGraphDatas(resp?.data?.payload);
      } else {
        let resp = await ApiGet(`straper/getStraperRepoMonth`);
        setGraphDatas(resp?.data?.payload?.mixData);
      }
    } catch (error) {
      toast.error("Something went wrong");
    }
  ...
  const handleGetLink = async () => {
    try {
      ...
      if (token) {
        window.open(`https://intern.wepro-deutschland.de?${token}`);
      }
    } catch (error) {
      window.open(`https://intern.wepro-deutschland.de`);
    }
  };
  ...
  }, [id, firstDate, endendDate]);

  const todayDate = new Date();

  const getTopTenData = async () => {
    const todayDates = new Date();
    const formattedDate = todayDates?.toISOString().split("T")[0];
    try {
      let resp = await ApiGet(
        `vattenfall/getTopTenUserLead?startDate=${formattedDate}&endDate=${formattedDate}`
      );
      setTopTen(resp?.data?.payload)
  ...
  const leads = [
    {
      ...
      data: graphDatas
        ? [
            graphDatas?.[0]?.count || 0,
            graphDatas?.[1]?.count || 0,
            graphDatas?.[2]?.count || 0,
            graphDatas?.[3]?.count || 0,
            graphDatas?.[4]?.count || 0,
            graphDatas?.[5]?.count || 0,
            graphDatas?.[6]?.count || 0,
            graphDatas?.[7]?.count || 0,
            graphDatas?.[8]?.count || 0,
            graphDatas?.[9]?.count || 0,
            graphDatas?.[10]?.count || 0,
            graphDatas?.[11]?.count || 0,
          ]
        : Array(12).fill(0),
    },
  ];
  ...
  const updateLead = async () => {
    if (hanldeValidation()) {
      let body = {
        userLead: updatedLead,
      };

      setLoading(true);
      try {
        let resp = await ApiPut(`user/updateUserLead?id=${id}`, body);
        setLoading(false);
        setNewModel(!newmodel);
        toast.success("Monatsziel aktualisiert");
        setupdatedLead("");
        getLeadData();
      } catch (error) {
        toast.error("something went wrong");
  ...
  const statusData = async () => {
    try {
      let resp = await ApiGet(
        `straper/getStraperStatus?startDate=${
          startDate?.toISOString().split("T")[0]
        }${
          endDate ? `&endDate=${endDate?.toISOString().split("T")[0]}` : ""
        }${`&_id=${id || userInfo?._id}`}`
      );
      setStatusDatas(resp?.data?.payload);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const calculateStatusSummary = () => {
    const newStatusVatenData = statusDatas && statusDatas?.statusCountsVatt;

    let Open = 0;
    let Confirmed = 0;
    let Rejected = 0;

    if (newStatusVatenData && typeof newStatusVatenData === "object") {
      for (const [key, value] of Object.entries(newStatusVatenData)) {
        if (key && typeof value === "number") {
          ...
          if (
            ...
            Confirmed += value;
          } else if (["STORNO", "ABSAGE"].includes(key)) {
            Rejected += value;
          }
        }
      }
    ...
    return { Open, Confirmed, Rejected };
  };
  const statusSummary = calculateStatusSummary();

  const calculateStatusSummary2 = () => {
    const newStatusVatenData = newData && newData?.statusCountsVatt;
    ...
    return { Open: Opens, Confirmed: Confirmeds, Rejected: Rejecteds };
  };

  const statusSummary2 = calculateStatusSummary2();

  const obj1 = statusSummary2;
  const obj2 = newData?.statusCounts != {} && newData?.statusCounts;
  ...
  const newStatusDats = {
    ...
    datasets: [
      {
        data: [combined?.Open || 100, combined?.Confirmed, combined?.Rejected],
        backgroundColor: [
          combined?.Open ? "#E8A56A" : "#E0E0E0",
          "#7CC5AF",
          "#EA7575",
        ],
        borderWidth: 0,
  ...
  const newobj1 = leadData?.currentMonth;
  const newobj2 = leadData?.currentMonthVatt;

  const totalLeads = newobj1?.lead + newobj2?.lead;
  const achiveLead = newobj1?.monthly + newobj2?.monthly;
  ...
  const ourdata = {
    ...
    datasets: [
      {
        data:
          totalLeads > achiveLead
            ? [achiveLead, totalLeads - achiveLead]
            : totalLeads > 0 && achiveLead > 0 && totalLeads < achiveLead
            ? [achiveLead, 0]
            : [0, 100],
    ...
    <>
      <div className="eingereichteLeads-page-all-content-alignment">
        <div className="page-header-alignment">
          <h2>Leads</h2>
          <div className="btn-new-alig">
            {id && (
              <button onClick={() => setNewModel(!newmodel)}>
                <svg
          ...
          <div className="first-grid-items-pp">
            ...
            <div className="sub-box-pp">
              ...
              <div className="right-content-pp">
                {/* <p className="wattefox-data">
                  Wattfox {statusDatas?.statusCounts?.Open || 0}
                </p> */}
                <p className="wattefox-data">
            ...
            <div className="sub-box-pp sub-box-margin-pp">
              ...
              <div className="right-content-pp">
                {/* <p className="wattefox-datas">
                  Wattfox {statusDatas?.statusCounts?.Confirmed || 0}
                </p> */}
                <p className="wattefox-datas">
            ...
            <div className="sub-box-pp sub-box-color-pp">
              ...
              <div className="right-content-pp">
                {/* <p className="wattefox-data">
                  Wattfox {statusDatas?.statusCounts?.Rejected || 0}
                </p> */}
                <p className="wattefox-data">
        ...
        <div className="sec-grid">
          <div className="sec-grid-items">
            ...
            <div className="besten-box">
              <h3>BESTEN</h3>
              <div className="box-center-alignment-text">
                <p>
                  DES TAGES <br />
                  ...
                  {topTen?.map((item, i) => {
                    return (
                      <div className="text-grid">
                        <h4>{i + 1}</h4>
                        <h5>
                          <b>{item?.benutzername}</b>   {item?.totalCount} LEADS
                        </h5>
                      </div>
                    );
                  })}
          ...
          <div className="sec-grid-items-main">
            <div className="sec-grid-items">
              ...
              <div className="white-box-chart">
                <div className="white-chart-circle">
                  {newData?.statusCounts != {} ||
                  newData?.statusCounts?.Open > 0 ||
                  newData?.statusCounts?.Confirmed > 0 ||
                  newData?.statusCounts?.Rejected > 0 ? (
                ...
                <div className="new-chart-image imag-alig">
                  <div className="data">
                    ...
                    <p>
                      {" "}
                      Offen - {combined?.Open || 0} (
                      {combined?.Open > 0
                        ? (
                            (Number(combined?.Open || 0) /
                              (Number(combined?.Open || 0) +
                                Number(combined?.Confirmed || 0) +
                                Number(combined?.Rejected || 0))) *
                            100
                          ).toFixed(0) + "%"
                        : 0 + "%"}
                      )
                  ...
                  <div className="data2">
                    ...
                    <p>
                      {" "}
                      Bestätigt - {combined?.Confirmed || 0} (
                      {combined?.Confirmed > 0
                        ? (
                            (Number(combined?.Confirmed) /
                              (Number(combined?.Open || 0) +
                                Number(combined?.Confirmed || 0) +
                                Number(combined?.Rejected || 0))) *
                            100
                  ...
                  <div className="data3">
                    ...
                    <p>
                      {" "}
                      Abgelehnt - {combined?.Rejected || 0} (
                      {combined?.Rejected > 0
                        ? (
                            (Number(combined?.Rejected) /
                              (Number(combined?.Open || 0) +
                                Number(combined?.Confirmed || 0) +
                                Number(combined?.Rejected || 0))) *
                            100
              ...
              <Calendar
                ...
                onMonthChange={(date) => {
                  setDates({
                    ...
                    endDate: getEndDateOfMonth(date.format("YYYY-MM-DD")),
                  });
                }}
                onYearChange={(date) =>
                  setDates({
                ...
                mapDays={({ date }) => {
                  ...
                  if (dateIsAvailableTravel(strDate))
                    className = "ready-to-travel";
